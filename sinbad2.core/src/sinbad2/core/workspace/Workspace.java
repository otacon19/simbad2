...
public class Workspace implements IFileChangeListener {
	...
	public void setFileName(String fileName) {
		...
		WorkspaceChangeEvent event = new WorkspaceChangeEvent(EWorkspaceChange.NEW_FILE, 
				oldValue, _fileName);
	...
	public void close() {
		...
		event = new WorkspaceChangeEvent(EWorkspaceChange.PRE_WORKSPACE_CLOSE
				, oldValue, _content);
		notifyWorkspaceChange(event);
		...
		if(_doClose) {
			...
			event = new WorkspaceChangeEvent(EWorkspaceChange.POST_WORKSPACE_CLOSE
					, oldValue, _content);
			notifyWorkspaceChange(event);
	...
	public void updateHashCode() {
		...
		if(oldValue != _currentHashCode) {
			WorkspaceChangeEvent event = new WorkspaceChangeEvent(
					EWorkspaceChange.HASH_CODE_MODIFIED, oldValue, _currentHashCode);
			notifyWorkspaceChange(event);
	...
	public void fileChanged(File file) {
		...
		if(!file.exists()) {
			FileMonitor.getInstance().removeFileChangeListener(this, file);
