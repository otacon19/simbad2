...
public class ExcelManager implements IExportListener, IImportListener {
	...
	private List<XSSFSheet> _expertsSheets;
	...
	private void createExpertAssessments(Expert e, Map<ValuationKey, Valuation> valuations) {
		...
		cell.setCellValue(e.getCanonicalId());
				...
				if(!c.hasSubcriteria()) {
					for (ValuationKey vk : valuations.keySet()) {
						if (vk.getExpert().equals(e) && vk.getAlternative().equals(a) && vk.getCriterion().equals(c)) {
							...
							cell = rowAlternative.createCell(3 + numCriterion);
	...
	public void notifyExportData() {
		...
		ExportHandler.unregisterExportListener(this);
	}
	...
	public void createExcelFile() {
		...
		if(!experts.isEmpty() && !alternatives.isEmpty() && !criteria.isEmpty()) {
			
			createGatheringInformation();
	...
	private void createGatheringInformation() {
		for (Expert e : _experts) {
			if(!e.hasChildren()) {
				_sheet = _workbook.createSheet(e.getCanonicalId());
				createInformationTemplate(e);
			}
		}
	}
	
	private void createInformationTemplate(Expert e) {
		int rowCountExpertsCriteria = 2, rowCountAlternatives = 3, columnCountCriteria = 3;
	...
	public void notifyImportData() {
		...
		ImportHandler.unregisterImportListener(this);
	}
	...
	private void readExcelFile() {
		...
		_expertsSheets = new LinkedList<XSSFSheet>();
		for(int i = 0; i < _workbook.getNumberOfSheets(); ++i) {
			_expertsSheets.add(_workbook.getSheetAt(i));
		}
	}

	private void importInformation() {
		ProblemElementsSet elementsSet = ProblemElementsManager.getInstance().getActiveElementSet();
		
		importExperts(elementsSet);
		importAlternatives(elementsSet);
	}

	private void importExperts(ProblemElementsSet elementsSet) {
		String[] experts;
		for(XSSFSheet s: _expertsSheets) {
			String name = s.getSheetName();
			experts = name.split(">");
			if(experts.length > 1) {
				for(int i = 0; i < experts.length - 1; ++i) {
					Expert parent = new Expert(experts[i]);
					elementsSet.addExpert(parent, false, false);
					Expert child = new Expert(experts[i + 1]);
					elementsSet.addExpert(child, true, false);
				}
			} else {
				Expert parent = new Expert(experts[0]);
				elementsSet.addExpert(parent, false, false);
			}
		}
	}
	
	private void importAlternatives(ProblemElementsSet elementsSet) {
		XSSFSheet anySheet = _expertsSheets.get(0);
		int numRow = 3;
		while(anySheet.getRow(numRow) != null) {
			elementsSet.addAlternative(new Alternative(anySheet.getRow(numRow).getCell(4).getStringCellValue()), false);
			numRow++;
		}
	}
}
